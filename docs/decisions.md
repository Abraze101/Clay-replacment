...
- **Status:** accepted and LIVE — the owner funded OpenRouter the same day (superseding the brief same-day interim note about using Anthropic/OpenAI instead): key `lead-engine-local` ($10 credit cap) installed in `.env`, `GENERATE_MODEL_PROVIDER=openrouter`, `minimax/minimax-m3` confirmed on the live model list, and the first real generation pass regenerated 20 live-canary call-note packs for $0.019 with all claims evidence-grounded. Apollo Basic was also purchased and probe-verified the same day (ADR-028's plan gate lifted: 25/25 live people-search rows, 0 credits).
- **Revisit trigger:** OpenRouter pricing/routing/availability problems for MiniMax models, the embedded assistant needing features OpenRouter does not pass through, or generation volume making the MiniMax cost delta material.
