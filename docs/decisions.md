...
- **Status:** accepted — **interim amendment (2026-07-13):** the owner defers the OpenRouter/MiniMax account for now and runs generation on Anthropic (Claude) or the owner's existing OpenAI API account instead; both adapters shipped at M5 behind the same interface, so this is only an env selection (`GENERATE_MODEL_PROVIDER` + the matching key). The OpenRouter adapter stays ready for when MiniMax economics matter (its per-lead cost is ~10x cheaper than Claude/GPT at list prices, all negligible at current volumes).
