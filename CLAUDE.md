...
- Record vendor/library decisions as ADR entries in `docs/decisions.md` with revisit triggers. Platform-scraping prohibitions (LinkedIn, Google Maps, SERP) are permanent guardrails, not relitigatable ADRs.
...
- Store sanitized provider fixtures for tests; CI must not use live Apollo, MiniMax, OpenAI, Anthropic, LinkedIn, or HubSpot credentials.
